#ifndef FRUSTUM_REG_2D_HPP
#define FRUSTUM_REG_2D_HPP
...
struct GivenKOutsideImgError2D {
    GivenKOutsideImgError2D(double point_x, double point_y, double point_z,
    ...
    bool operator()(const T* const camera,
        ...
        T p[3];
        T angle_axis[3] = {T(0.0), camera[0], T(0.0)};
        ceres::AngleAxisRotatePoint(angle_axis, point, p);
        // camera[1,2,3] are the translation.
        p[0] += camera[1];
        p[1] += camera[2];
        p[2] += camera[3];
    ...
    static ceres::CostFunction* Create(double point_x, double point_y, double point_z,
                                       double fx, double fy, double cx, double cy,
                                       double H, double W) {
        return (new ceres::AutoDiffCostFunction<GivenKOutsideImgError2D, 1, 4>(
                    new GivenKOutsideImgError2D(point_x, point_y, point_z,
                                              fx, fy, cx, cy,
                                              H, W)));
    }
    double point_x;
...
// label == 1, that is, the point should be inside the image plane
struct GivenKInsideImgError2D {
    GivenKInsideImgError2D(double point_x, double point_y, double point_z,
                         double fx, double fy, double cx, double cy,
                         double H, double W)
        : point_x(point_x)
        , point_y(point_y)
    ...
    static ceres::CostFunction* Create(double point_x, double point_y, double point_z,
                                       double fx, double fy, double cx, double cy,
                                       double H, double W) {
        return (new ceres::AutoDiffCostFunction<GivenKInsideImgError2D, 3, 4>(
                    new GivenKInsideImgError2D(point_x, point_y, point_z,
                                             fx, fy, cx, cy,
                                             H, W)));
    }
    double point_x;
